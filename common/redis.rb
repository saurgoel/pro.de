...
namespace :redis do
  ...
  task :start do
    comment %{Starting Redis}
    command %{service redis start}
  end

  task :stop do
    comment %{Stopping Redis}
    command %{service redis stop}
  end

  task :test do
    comment %{Testing Redis}
    command %{redis-cli -h REMOTE.HOST ping}
    command %{redis-cli -h redis.r1.getamber.com ping}
  end
end
...
# execute "echo 'vm.overcommit_memory = 1' >> /etc/sysctl.conf"

# replaces apendonly no with appendonly yes in redis
# execute "sed 's/appendonly no/appendonly yes/' /etc/redis.conf"
...
# =========== Easy Copy
# enter the redis host and port of the master
...
# Other issues
# /var/log/redis and in 
# /var/lib/appendonly.aof there were hackers that were making flushall after some time again and again
# grep for flushall and see the -A 10 and -B 10 
